...
:math:`p_{AE}(f_B, f_W, T, L)`, tabulates the Shannon
entropies\ :footcite:`shannon1948` of every marginal, and combines them.
...
square-root rule :math:`N_{bins} = 1 + \lceil\sqrt{n}\rceil` over the living
instances (:math:`f_B f_W > 10^{-7}`); luminosities with fewer than two living
instances are ``NaN``.
...
:func:`~pyEDW.metrics.intra_environment_information` also take a table stacked over a
2-D sweep, shape ``(n_bandwidth, n_lum, 16)``, and broadcast over the leading axis, so a full
...
Negative :math:`C(a_1{:}a_2\|E)` means the environment synergistically enhances
the correlation between the two daisy species.
